...
class Wire {
    ...
    traverse(input: string[]) {
        input.forEach(step => {
            // console.log(step);
            const repeat = Number(step.substr(1));
    ...
    findNearestIntersection(array: string[]) {
        let lowest: number = 0;
        array.forEach(step => {
            const x: number = Math.abs(Number(step.split(',')[0]));
            const y: number = Math.abs(Number(step.split(',')[1]));

            const total: number = x + y;

            if (total < lowest || lowest === 0) {
                lowest = total;
            }
    ...
    findShortestIntersection(array: string[]) {

    }
}

const wireRed = new Wire(inputRed);
const wireBlue = new Wire(inputBlue);

wireRed.compare(wireBlue);